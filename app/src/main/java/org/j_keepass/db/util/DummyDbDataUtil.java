...
public class DummyDbDataUtil {
    public Database<?, ?, ?, ?> getDummyDatabase() {
        ...
        if (Utils.isLogFlagEnabled()) {
            loopAndAdd(database, currentCalender, rootGroup);
        }

        return database;
    }

    private void loopAndAdd(Database<?, ?, ?, ?> database, Calendar currentCalender, Group rootGroup) {
        for (int i = 5; i < 50; i++) {
